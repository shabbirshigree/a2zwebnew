...
export default function Gallery({ title, images, isObjectArray }) {
  ...
  return (
    <div className="my-16 px-4">
      {/* گیلری کا عنوان */}
      <h2 className="text-2xl md:text-4xl font-bold mb-10 text-center text-[#0b314d] persian-text border-b-2 border-[#D4AF37] pb-3 inline-block pl-6">{title}</h2>
            ...
            >
              ...
              {isObjectArray && (
                <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 to-transparent p-3 text-center opacity-0 group-hover:opacity-100 transition-opacity">
                  <p className="text-white text-sm font-medium persian-text leading-tight">{img.title}</p>
                </div>
              )}
            </div>
          );
      ...
      {currentIndex !== null && (
        ...
        >
          ...
          >
            ...
            {isObjectArray && (
              <div className="absolute -bottom-10 text-white text-xl font-semibold text-center w-full persian-text">
