...
import { quranVideos } from "../noor-ul-quran/projectData";export default function FarsiProjectPage() {
  ...
  const shareFromPopup = (key) => {
    ...
    const text = `پروژه نورالقرآن را به اشتراک بگذارید (${key})`;
    ...
    <main className="min-h-screen bg-[#050505] text-white overflow-x-hidden font-sans">
      ...
      <section className="bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-[#0f2a40] via-[#050505] to-[#000000] py-12 md:py-16 text-center relative border-b border-[#D4AF37]/20">
        <div className="container mx-auto px-4 relative z-10">
          <h1 className="text-4xl md:text-6xl font-extrabold text-[#D4AF37] mb-3 tracking-wide">پروژه نورالقرآن</h1>
          <h2 className="text-base md:text-xl text-white/80 font-sans tracking-widest mb-3">(قرآن تصویری)</h2>
          <div className="w-16 h-0.5 bg-[#D4AF37] mx-auto mb-4"></div>
          <p className="text-sm md:text-lg text-[#fff7cc] font-light max-w-2xl mx-auto leading-relaxed border-t border-b border-[#D4AF37]/30 py-3" dir="rtl">
      ...
      {isLocalVideoOpen && (
        <div className="fixed inset-0 bg-black/95 z-[100] flex flex-col items-center justify-center p-4" onClick={() => setIsLocalVideoOpen(false)}>
          <div className="w-full max-w-4xl relative" onClick={(e) => e.stopPropagation()}>
            <button className="absolute -top-12 right-0 bg-red-600 text-white px-4 py-2 rounded-full" onClick={() => setIsLocalVideoOpen(false)}><FaTimes /> بستن</button>
