...
import { CHANNELS } from './channelsData-fa';
...
export default function ChannelsPage() {
  ...
  return (
    <main suppressHydrationWarning className="min-h-screen bg-slate-50 flex flex-col overflow-x-hidden" dir="rtl">
      <Navbar />
      <HeroSlider />

      {/* ٹائٹل سیکشن */}
      <section className="bg-white py-10 text-center relative z-10">
        <h1 className="text-3xl md:text-5xl font-bold text-[#0f4c75] persian-text mb-2">مرکزهای رسانه‌ای</h1>
      ...
      <section className="container mx-auto px-4 -mt-6 relative z-20 mb-10">
        <div className="flex items-center bg-white border border-[#D4AF37]/50 rounded-full px-4 py-2 shadow-lg max-w-xl mx-auto">
          <input
            ...
            className="flex-1 outline-none text-right px-2 bg-transparent text-[#0f4c75] persian-text"
          />
          ...
          {filtered.map((channel, i) => (
            <div key={i} className="bg-white rounded-2xl overflow-hidden shadow-md border border-gray-100 group hover:shadow-2xl transition duration-300">
              <a href={channel.href} target="_blank" rel="noopener noreferrer" className="block">
                <div className={`h-32 bg-gradient-to-bl ${channel.color} relative flex items-center justify-center`}>
              ...
              <div className="p-5 text-right" dir="rtl">
                <h2 className="text-xl font-bold text-[#0f4c75] persian-text mb-1">{channel.title}</h2>
                <p className="text-[#D4AF37] text-xs font-bold mb-3 font-sans" dir="ltr">{channel.handle}</p>
                <p className="text-gray-600 text-sm h-12 line-clamp-2 mb-4 persian-text">{channel.desc}</p>
      ...
      <section className="container mx-auto px-4 py-10">
        <div className="max-w-4xl mx-auto bg-[#0f4c75] rounded-3xl p-8 text-white border-2 border-[#D4AF37] text-center relative overflow-hidden">
          <div className="relative z-10">
            <h2 className="text-xl md:text-4xl font-bold mb-3 persian-text text-[#D4AF37] text-center">ما را دنبال کنید</h2>
            <p className="text-sm md:text-xl mb-8 opacity-90 persian-text text-center">در شبکه‌های اجتماعی با ما همراه شوید</p>
