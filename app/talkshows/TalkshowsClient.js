...
function TalkshowsContent() {
    ...
    // 🔴 آٹو پلے ویڈیو اگر URL میں v موجود ہو
    useEffect(() => {
    ...
    const handleShare = (vid) => {
        ...
        const shareText = `*${vid.title}*\n\nنور پروڈکشنز کی یہ خصوصی ویڈیو دیکھنے کے لیے لنک پر کلک کریں 👇\n\n${shareUrl}`;
            ...
            return (
                <div key={i} className="relative group rounded-xl overflow-hidden border-2 border-white/20 hover:border-[#D4AF37] shadow-lg bg-black aspect-video flex flex-col">
                    ...
                    <div className="absolute bottom-0 w-full bg-gradient-to-t from-black via-black/90 to-transparent p-3 flex items-end justify-between gap-2">
                        <h4 className="text-white font-bold text-xs md:text-sm urdu-text leading-tight flex-1 line-clamp-2">{vid.title}</h4>
                        <button 
                            onClick={(e) => {
                                e.stopPropagation();
                                handleShare(vid);
