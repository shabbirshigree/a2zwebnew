...
export default function ArticlesPage() {
  ...
  // 🔴 بٹنوں کی نئی ترتیب (آپ کی ہدایت کے مطابق)
  const categories = [
        ...
        {!selectedArticle ? (
          <>
            ...
            <div className="bg-white p-5 md:p-8 rounded-2xl shadow-lg border border-[#D4AF37]/30 mb-10 text-center relative overflow-hidden" dir="rtl">
              ...
              {/* 🔴 بٹنوں کا سائز اور گیپ چھوٹا کر دیا گیا ہے */}
              <div className="flex flex-wrap justify-center gap-2">
                {categories.map(cat => (
                  <button 
                    key={cat.id} 
                    onClick={() => setFilterCategory(cat.id)}
                    className={`px-3 py-1.5 md:px-4 md:py-2 rounded-full font-bold urdu-text text-xs md:text-sm transition-all duration-300 shadow-sm border ${
                  ...
                  >
                    <div className="h-48 overflow-hidden bg-gray-100 relative">
                      ...
                      <div className="absolute top-3 right-3 bg-[#D4AF37] text-[#0b314d] text-xs font-bold px-3 py-1 rounded-full shadow-md">
                        {categories.find(c => c.id === article.category)?.label.replace(/[^a-zA-Zآ-ی]/g, '').trim()}
                      </div>
                    </div>
          ...
          <div className="max-w-4xl mx-auto bg-white p-6 md:p-12 rounded-3xl shadow-2xl border-t-8 border-[#D4AF37]">
            ...
            <div className="text-right" dir={selectedArticle.category === 'english' ? 'ltr' : 'rtl'}>
              <h1 className="text-3xl md:text-5xl text-[#0b314d] mb-6 font-bold leading-tight" style={{ fontFamily: selectedArticle.category === 'english' ? 'sans-serif' : "'Jameel Noori Nastaleeq', serif" }}>
              ...
              <div 
                ...
                style={{ fontFamily: selectedArticle.category === 'english' ? 'sans-serif' : "'Jameel Noori Nastaleeq', serif" }}
