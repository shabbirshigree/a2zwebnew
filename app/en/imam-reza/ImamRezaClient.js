...
export default function ImamRezaClient() {
  ...
  const handleShare = (e, vid) => {
    ...
    if (navigator.share) {
      navigator.share({
        title: vid.title,
        text: `Watch this video: ${vid.title}`,
        url: shareUrl,
      }).catch(() => {});
    } else {
    ...
    <main className="min-h-screen bg-[#f8f9fa] text-gray-800 relative overflow-hidden font-amiri">
      ...
      <div className="relative z-10 container mx-auto px-4 pt-16 pb-8 text-center">
        <div className="bg-white/80 backdrop-blur-md border-2 border-[#D4AF37] rounded-[2rem] p-6 md:p-10 shadow-xl max-w-5xl mx-auto animate-float">
           ...
           <h1 className="text-2xl md:text-5xl font-extrabold text-[#0f4c75] mb-2 font-extra-bold">Haji Shabbir Ahmed Shigri — Servant of Imam Reza</h1>
           <h2 className="text-lg md:text-2xl text-[#D4AF37] font-bold">A spiritual journey of faith, service, and cultural duty</h2>
           <div className="w-full h-[2px] bg-gradient-to-r from-transparent via-[#D4AF37] to-transparent my-6 opacity-50"></div>
           <div className="text-gray-900 text-sm md:text-xl leading-loose text-justify md:text-center max-w-4xl mx-auto space-y-4 font-extra-bold" dir="ltr">
             <p className="font-extrabold text-[#0f4c75] text-lg md:text-2xl">"Where kings and rulers bow, this is a respected gathering..."</p>
             <p>Welcome to the dedicated page of service and spiritual mission at the Holy Shrine of Imam Reza (A.S). This page showcases the magnificent journey of Haji Shabbir Ahmed Shigri representing beautiful heritage, religious love, and cultural service.</p>
             <p className="bg-[#f8f9fa] p-4 rounded-xl border-l-4 border-[#D4AF37] shadow-sm"><strong className="text-[#0f4c75] text-lg">First Light: "Zarih-e-Noor" (2002)</strong><br />In 2002, he documented the holy construction of the current Zarih. In 2011, Astan Quds Razavi honored him with the title <strong className="text-[#D4AF37]">"Servant of Imam Reza"</strong>.</p>
           </div>
        </div>
      </div>
      ...
      <div className="relative z-10 container mx-auto px-4 py-8">
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4 max-w-7xl mx-auto" dir="ltr">
      ...
      <div id="docs" className="relative z-10 container mx-auto px-4 py-10">
         <div className="text-center mb-8"><h2 className="text-2xl md:text-3xl font-extrabold text-[#0f4c75] border-b-4 border-[#D4AF37] inline-block pb-2">Special Documentaries & Videos</h2></div>
         <div className="grid grid-cols-1 md:grid-cols-3 gap-4" dir="ltr">
            {documentaryVideos.map((vid, idx) => (
               <div key={idx} onClick={() => setActiveVideo(vid.link)} className="bg-white border-l-4 border-[#D4AF37] p-4 rounded-xl shadow-sm flex items-center gap-3 cursor-pointer hover:bg-[#fff9e6] transition-colors group relative">
                  ...
                  <button onClick={(e) => handleShare(e, vid)} className="absolute top-2 right-2 text-gray-400 hover:text-[#D4AF37] transition-colors"><FaShareAlt size={14} /></button>
               </div>
            ))}
         </div>
      </div>
      ...
      <div id="tabarrukat" className="relative z-10 container mx-auto px-4 py-10 bg-[#D4AF37]/10 rounded-[3rem]">
         <div className="text-center mb-8"><h2 className="text-2xl md:text-3xl font-extrabold text-[#8a6200] border-b-4 border-white inline-block pb-2">Distribution of Tabarrukat</h2></div>
         <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl mx-auto" dir="ltr">
            {tabarrukatVideos.map((vid, idx) => (
               <div key={idx} onClick={() => setActiveVideo(vid.link)} className="bg-white p-6 rounded-2xl shadow-lg border-2 border-[#D4AF37] flex flex-col items-center gap-3 cursor-pointer hover:scale-105 transition-transform group text-center relative">
                  ...
                  <span className="text-sm text-gray-500 font-bold">Watch</span>
                  <button onClick={(e) => handleShare(e, vid)} className="absolute top-4 right-4 text-gray-400 hover:text-[#D4AF37] transition-colors"><FaShareAlt size={18} /></button>
               </div>
            ))}
         </div>
      </div>
      ...
      <div id="programs" className="relative z-10 container mx-auto px-4 py-10">
         <div className="text-center mb-8"><h2 className="text-2xl md:text-3xl font-extrabold text-[#0f4c75] border-b-4 border-[#D4AF37] inline-block pb-2">Shrine Programs & Live Ziyarat</h2></div>
         <div className="grid grid-cols-1 md:grid-cols-2 gap-4" dir="ltr">
            {programVideos.map((vid, idx) => (
               <div key={idx} onClick={() => setActiveVideo(vid.link)} className="bg-white border-l-4 border-[#0f4c75] p-4 rounded-xl shadow-sm flex items-center gap-3 cursor-pointer hover:bg-[#e6f4ff] transition-colors group relative">
      ...
      {liveParticipationVideos.length > 0 && (
          <div id="liveParticipations" className="relative z-10 container mx-auto px-4 py-10 bg-[#0f4c75]/5 border-y-4 border-[#D4AF37] my-8">
             <div className="text-center mb-8"><h2 className="text-2xl md:text-3xl font-extrabold text-[#0f4c75] border-b-4 border-[#D4AF37] inline-block pb-2">Live Program Participation</h2></div>
             <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-5xl mx-auto" dir="ltr">
                {liveParticipationVideos.map((vid, idx) => (
                   <div key={idx} onClick={() => setActiveVideo(vid.link)} className="bg-white p-6 rounded-2xl shadow-xl border border-gray-200 flex flex-col items-center gap-4 cursor-pointer hover:-translate-y-2 hover:shadow-2xl transition-all group text-center relative">
                      ...
                      <span className="bg-[#D4AF37] text-white px-4 py-1 rounded-full text-xs font-bold mt-2">Watch</span>
                      <button onClick={(e) => handleShare(e, vid)} className="absolute top-4 right-4 text-gray-400 hover:text-[#D4AF37] transition-colors"><FaShareAlt size={18} /></button>
                   </div>
                ))}
             </div>
      ...
      <div id="manqabat" className="relative z-10 container mx-auto px-4 py-10">
         <div className="text-center mb-8"><h2 className="text-2xl md:text-3xl font-extrabold text-[#0f4c75] border-b-4 border-[#D4AF37] inline-block pb-2">Manqabats & Hymns</h2></div>
         <div className="grid grid-cols-1 md:grid-cols-3 gap-4" dir="ltr">
      ...
      <div id="services" className="relative z-10 container mx-auto px-4 py-12 bg-white">
         <div className="text-center mb-10"><h2 className="text-2xl md:text-3xl font-extrabold text-[#0f4c75] border-b-4 border-[#D4AF37] inline-block pb-2">Online Services & Webinars</h2></div>
         <div className="max-w-5xl mx-auto bg-white rounded-3xl shadow-2xl border-2 border-[#D4AF37] overflow-hidden flex flex-col md:row" dir="ltr">
            <div className="md:w-1/2 relative h-64 md:h-auto">
               ...
               <div className="absolute top-4 right-4 bg-red-600 text-white px-4 py-1 rounded-full text-xs font-bold shadow-md animate-pulse">Live Webinar</div>
            </div>
            <div className="md:w-1/2 p-8 flex flex-col justify-center space-y-4">
               <h3 className="text-2xl font-extrabold text-[#0f4c75] leading-tight">Hussaini Way and the 12-Day Battle of Truth and Falsehood</h3>
               <p className="text-gray-600 font-bold border-b pb-2">A special session organized in collaboration with Astan Quds Razavi and Noor Productions.</p>
               <div className="space-y-3 mt-2">
                  <div className="flex items-center gap-3 text-gray-700"><FaCalendarCheck className="text-[#D4AF37] text-xl" /><span className="font-bold">Date: Wednesday, July 30, 2025</span></div>
                  <div className="flex items-center gap-3 text-gray-700"><FaClock className="text-[#D4AF37] text-xl" /><span className="font-bold">Time: 15:30 to 16:30</span></div>
                  <div className="flex items-center gap-3 text-gray-700"><FaLaptop className="text-[#D4AF37] text-xl" /><span className="font-bold">Location: Online (Video Link)</span></div>
               </div>
            </div>
         </div>
      </div>
      ...
      {activeVideo && (
        <div className="fixed inset-0 z-[100] bg-black/95 flex items-center justify-center p-4 backdrop-blur-md">
          <button onClick={() => {
            setActiveVideo(null);
            router.push('/en/imam-reza', { scroll: false });
          }} className="absolute top-5 right-5 text-[#D4AF37] text-4xl hover:text-red-500 transition-all z-[101]"><FaTimes /></button>
      ...
      {selectedArticle && (
        <div className="fixed inset-0 z-[100] bg-black/80 flex items-center justify-center p-4 backdrop-blur-sm overflow-y-auto">
            <div className="bg-white w-full max-w-4xl rounded-2xl shadow-2xl relative overflow-hidden flex flex-col max-h-[90vh]">
                ...
                <div className="p-6 overflow-y-auto" dir="ltr">
