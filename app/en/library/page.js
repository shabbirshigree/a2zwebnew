...
import { BOOKS_DATA, AUTHOR_REVIEW } from './data';
...
const UrduFlipBook = dynamic(() => import('../../library/UrduFlipBook'), {
  ...
  loading: () => (
    <div className="flex flex-col items-center justify-center p-20 gap-4">
      ...
      <div className="text-[#D4AF37] text-center text-xl">Please Wait...</div>
      <div className="text-[#D4AF37] text-sm opacity-80">Haji Shabbir Ahmed Shigri</div>
    </div>
  )
});
...
export default function LibraryPage() {
  ...
  // Auto scroll for external visitors
  useEffect(() => {
  ...
  const filteredBooks = BOOKS_DATA.filter(book =>
    ...
    book.descEn.toLowerCase().includes(searchQuery.toLowerCase())
  );
  ...
  const handlePlayMedia = (url) => {
    if (url) {
      ...
      alert("Media for this book is not available.");
    }
  };
  ...
  const handleShare = async (book, action) => {
    ...
    if (action.type === 'read') {
      ...
      shareDetails = `*${book.title}*\n\n✍️ Author: Hajji Shabbir Ahmed Shigri\n\nRead this amazing book by clicking the link below 👇\n\n${shareUrl}`;
    } else if (action.type === 'video') {
      shareUrl = action.url;
      shareDetails = `*${book.title}*\n\nWatch the amazing "Video Analysis" 👇\n\n${shareUrl}`;
    } else if (action.type === 'audio') {
      shareUrl = action.url;
      shareDetails = `*${book.title}*\n\nListen to the excellent "Audio Analysis" 👇\n\n${shareUrl}`;
    }
    ...
    <main className="min-h-screen bg-[#050505] text-white overflow-x-hidden font-sans">
      ...
      <section className="bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-[#1a1005] via-[#050505] to-[#000000] py-16 text-center border-b border-[#D4AF37]/30">
        <div className="container mx-auto px-4 relative z-10">
          <h1 className="text-4xl md:text-5xl font-extrabold text-[#D4AF37] mb-4 drop-shadow-md">Treasury of Knowledge and Wisdom</h1>
          ...
          <div className="max-w-md mx-auto relative group">
            ...
            <input
              ...
              className="w-full bg-[#111] border border-[#D4AF37]/20 rounded-full py-3 px-12 text-left focus:outline-none focus:border-[#D4AF37] transition-all"
              value={searchQuery}
        ...
        <div className="bg-[#0a0a0a] rounded-3xl p-6 shadow-2xl border border-[#D4AF37]/30 max-w-4xl mx-auto relative overflow-hidden flex flex-col md:flex-row items-center gap-8">
          ...
          <div className="flex-1 text-center md:text-left">
            ...
            <div className="flex flex-wrap justify-center md:justify-start gap-3">
              <button onClick={() => handlePlayMedia(AUTHOR_REVIEW.videoUrl)} className="flex items-center gap-2 bg-red-700 text-white px-6 py-2 rounded-full font-bold text-xs hover:bg-red-600 transition-all"><FaFilm /> Watch Video</button>
              <button onClick={() => handlePlayMedia(AUTHOR_REVIEW.audioUrl)} className="flex items-center gap-2 bg-[#D4AF37] text-black px-6 py-2 rounded-full font-bold text-xs hover:bg-yellow-500 transition-all"><FaHeadphones /> Listen Audio</button>
            </div>
          </div>
        </div>
      </section>
        ...
        {filteredBooks.map((book) => (
          ...
          >
            ...
            <div className="flex-1 text-left flex flex-col justify-center">
              ...
              <h2 className="text-2xl md:text-4xl font-bold text-white mb-6 leading-tight">{book.title}</h2>
              <div className="w-20 h-1 bg-[#D4AF37]/50 mb-6 rounded-full"></div>
              <p className="text-gray-300 text-sm md:text-lg leading-[2.2] text-justify font-light">{book.descEn}</p>
            </div>
          </div>
        ))}
      </section>
      ...
      {videoModalOpen && (
        <div className="fixed inset-0 bg-black/95 z-[100] flex flex-col items-center justify-center p-4 backdrop-blur-sm" onClick={() => setVideoModalOpen(false)}>
          <div className="w-full max-w-3xl relative" onClick={e => e.stopPropagation()}>
            <button className="absolute -top-14 right-0 bg-red-600 hover:bg-red-700 text-white px-6 py-2 rounded-full flex items-center gap-2 text-sm font-bold z-50 shadow-xl" onClick={() => setVideoModalOpen(false)}>
              <FaTimes /> Close
            </button>
            <div className="rounded-3xl border-4 border-[#D4AF37]/50 overflow-hidden shadow-[0_0_50px_rgba(212,175,55,0.3)] bg-black">
              {mediaUrl.includes('.mp3') || mediaUrl.includes('podcast') ? (
                <div className="p-12 md:p-20 flex flex-col items-center text-center">
                  ...
                  <h3 className="text-white text-2xl mb-8">Audio Analysis Playing...</h3>
