...
function GalleryContentEN() {
  ...
  const handleShare = (e, item) => {
    ...
    const shareText = `*${item.tag}*\n\n${item.desc}\n\nClick the link below to view this memorable photo/video 👇\n\n${shareUrl}`;
    ...
    <main className="min-h-screen bg-[#f4f7f9] overflow-x-hidden font-sans">
      ...
      <section className="bg-[#0b314d] py-16 md:py-24 relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 relative z-10 flex flex-col items-center text-center">
          <h1 className="text-3xl md:text-6xl font-extrabold text-[#D4AF37] mb-4 drop-shadow-2xl">Documentary Journey</h1>
          <p className="text-white/80 text-xl md:text-2xl font-light tracking-widest max-w-3xl">A glimpse of 45 years of service through the windows of memories</p>
      ...
      <section className="container mx-auto px-4 -mt-10 relative z-20">
        <div className="flex items-center bg-white border-b-4 border-[#D4AF37] rounded-2xl px-6 py-4 shadow-2xl max-w-3xl mx-auto backdrop-blur-md bg-white/90">
          ...
          <input
            ...
            className="flex-1 outline-none text-gray-800 bg-transparent text-left text-lg md:text-xl placeholder:text-gray-400"
          />
        </div>
      </section>
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-[#0b314d] via-transparent to-transparent opacity-0 group-hover:opacity-95 transition-opacity duration-500 flex flex-col justify-end p-6">
                   <div className="translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
                      <div className="flex justify-between items-start mb-2">
                        <span className="bg-[#D4AF37] text-[#0b314d] px-3 py-1 rounded-md text-xs font-bold shadow-lg">{item.tag}</span>
                        <button 
                      ...
                      <p className="text-white font-bold text-lg leading-tight">{item.desc}</p>
                   </div>
                </div>
              </div>
            </div>
      ...
      {currentItem && (
        <div className="fixed inset-0 z-[100] bg-[#0b314d]/98 backdrop-blur-xl flex flex-col items-center justify-center transition-all duration-500 animate-in fade-in" onClick={closeLightbox}>
          ...
          <div className="mt-8 text-center text-white px-10 max-w-3xl">
            <h3 className="text-[#D4AF37] font-bold text-2xl md:text-3xl mb-2">{currentItem.tag}</h3>
            <p className="text-white/80 text-lg md:text-xl leading-relaxed">{currentItem.desc}</p>
