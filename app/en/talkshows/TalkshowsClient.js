...
import { FaYoutube, FaChevronDown, FaGlobe, FaWhatsapp, FaFacebook, FaTelegram, FaTiktok, FaShareAlt } from 'react-icons/fa';
import { FaXTwitter } from 'react-icons/fa6';

import Gallery from '../../talkshows/Gallery'; 
...
function TalkshowsContentEN() {
    ...
    const handleShare = (vid) => {
        ...
        const shareText = `*${vid.title}*\n\nWatch this special video by Noor Productions at the link below 👇\n\n${shareUrl}`;
    ...
    const renderVideos = (videosArray) => {
        return videosArray.map((vid, i) => {
            const videoId = getYouTubeId(vid.url);
            return (
                <div key={i} className="relative group rounded-xl overflow-hidden border-2 border-white/20 hover:border-[#D4AF37] shadow-lg bg-black aspect-video flex flex-col">
                    ...
                    <div className="absolute bottom-0 w-full bg-gradient-to-t from-black via-black/90 to-transparent p-3 flex items-end justify-between gap-2">
                        <h4 className="text-white font-bold text-xs md:text-sm leading-tight flex-1 line-clamp-2">{vid.title}</h4>
                        <button 
                            onClick={(e) => {
                                e.stopPropagation();
                                handleShare(vid);
                            }}
                            className="bg-[#D4AF37] text-[#0b314d] p-2 rounded-full hover:bg-white transition-colors shadow-lg"
                            title="Share"
                        >
                            <FaShareAlt size={14} />
                        </button>
        ...
        <div className="min-h-screen bg-[#f8f9fa] text-gray-800 font-sans">
            ...
            <section className="bg-gradient-to-b from-[#0b314d] to-[#0f4c75] text-white py-12 md:py-16 text-center relative border-b-4 border-[#D4AF37] shadow-xl">
                ...
                <div className="container mx-auto px-4 relative z-10">
                    <span className="bg-[#D4AF37] text-[#0b314d] font-bold px-4 py-1 rounded-full text-sm md:text-base mb-4 inline-block shadow-lg">Noor Productions Historic Presentation</span>
                    <h1 className="text-3xl md:text-5xl font-extrabold text-[#D4AF37] drop-shadow-lg mb-4 tracking-wide leading-tight">
                        {talkshowIntro.title}
                    </h1>
                    <h2 className="text-xl md:text-2xl text-[#fff7cc] font-light tracking-widest mb-4">
            ...
            <section className="container mx-auto px-4 py-16">
                <h2 className="text-2xl md:text-4xl font-bold text-[#0b314d] mb-12 text-center">Our Other Platforms & Channels</h2>
                    ...
                    {otherChannels.map((channel, idx) => (
                        <div key={idx} className="relative transition-all duration-300 hover:-translate-y-2 h-full">
                            ...
                            <div className="bg-white rounded-[2rem] p-6 shadow-lg border-2 border-[#D4AF37] h-full flex flex-col items-center text-center relative overflow-hidden group transition-all duration-500 hover:shadow-[0_0_20px_rgba(212,175,55,0.3)]">
                                ...
                                <h3 className="text-lg font-bold text-gray-800 mb-1 leading-tight group-hover:text-[#0b314d]">{channel.title}</h3>
                                <p className="text-[10px] font-bold text-blue-500 mb-3 opacity-70 tracking-tighter uppercase">{channel.handle || '@noorproduction'}</p>
                                <p className="text-gray-500 text-xs leading-relaxed mb-6 line-clamp-2">{channel.desc}</p>
                                <div className={`mt-auto w-full py-2.5 rounded-xl font-bold text-white text-sm shadow-md bg-gradient-to-r ${channel.color} transition-transform group-hover:scale-105`}>Visit</div>
                            </div>
                        </div>
                    ))}
                </div>
            ...
            {/* Noor Productions Special Introduction */}
            <section className="container mx-auto px-4 py-12">
                <div className="bg-white rounded-3xl shadow-2xl overflow-hidden border border-gray-200">
                    <div className="bg-gradient-to-r from-[#D4AF37] to-[#e6c875] p-6 text-center">
                        <h2 className="text-2xl md:text-4xl font-bold text-[#0b314d]">{talkshowIntro.noorProductionIntro.title}</h2>
                    </div>
                    <div className="p-8 md:p-12 text-gray-700 text-lg md:text-xl leading-loose space-y-6">
                        <p>{talkshowIntro.noorProductionIntro.text1}</p>
                        <p className="bg-gray-50 p-4 border-l-4 border-[#0b314d] rounded-lg italic text-[#0f4c75]">
            ...
            {/* Section 1: Introduction to Other Talk Shows */}
            <section className="container mx-auto px-4 pb-12">
                <div className="bg-white rounded-2xl shadow-lg border-t-4 border-[#D4AF37] p-6 md:p-10 mb-12 relative overflow-hidden">
                    ...
                    <p className="text-lg md:text-xl text-[#0b314d] leading-relaxed font-light text-justify drop-shadow-sm italic">
                        {talkshowIntro.quote}
                    </p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
                    {talkshowIntro.features.map((feature, index) => (
                        <div key={index} className="bg-white p-6 md:p-8 rounded-2xl shadow-md border-b-4 border-[#0b314d] hover:border-[#D4AF37] hover:shadow-xl transition-all duration-300">
                            <h3 className="text-xl font-bold text-[#0f4c75] mb-4 border-b border-gray-100 pb-3">
                                {feature.title}
                            </h3>
                            <p className="text-gray-700 text-sm md:text-base leading-relaxed text-justify font-light">
                                {feature.desc}
                            </p>
                        </div>
                    ))}
            ...
            <section className="bg-[#0b314d] py-12 md:py-16 relative border-y-4 border-[#D4AF37]">
                ...
                <div className="container mx-auto px-4 relative z-10">
                    <div className="text-center mb-12">
                        <h2 className="text-3xl md:text-4xl font-bold text-[#D4AF37] mb-6">🎬 Noor Productions Video Gallery</h2>

                        {/* Filters */}
                        <div className="flex flex-wrap justify-center gap-3">
                            <button
                                onClick={() => handleFilterChange('all')}
                                className={`px-4 py-2 rounded-full font-bold text-sm transition-all duration-300 ${filter === 'all' ? 'bg-[#D4AF37] text-[#0b314d] shadow-[0_0_15px_rgba(212,175,55,0.6)]' : 'bg-white/10 text-white hover:bg-white/20'
                                    }`}
                            >
                                All Videos
                            </button>
                            {categories.map(cat => (
                                <button
                                    ...
                                    className={`px-4 py-2 rounded-full font-bold text-sm transition-all duration-300 ${filter === cat ? 'bg-[#D4AF37] text-[#0b314d] shadow-[0_0_15px_rgba(212,175,55,0.6)]' : 'bg-white/10 text-white hover:bg-white/20'
                                        }`}
                                >
                                    {cat}
                                </button>
                            ...
                            categories.map(cat => {
                                ...
                                return (
                                    <div key={cat} className="mb-12">
                                        <h3 className="text-2xl md:text-3xl font-bold text-[#D4AF37] mb-6 border-b-2 border-white/20 pb-2 inline-block pl-6">
                                            {cat}
                                        </h3>
                    ...
                    <div className="flex justify-center mt-8 mb-10">
                        <button
                            ...
                            className="bg-transparent border-2 border-[#D4AF37] text-[#D4AF37] hover:bg-[#D4AF37] hover:text-[#0b314d] font-bold px-8 py-3 rounded-full flex items-center gap-2 transition-all duration-300 shadow-lg"
                        >
                            View More Videos <FaChevronDown />
                        </button>
                    </div>
                    ...
                    <div className="flex justify-center mt-8 border-t border-white/20 pt-8">
                        <a
                            ...
                            className="bg-red-600 hover:bg-red-700 text-white font-bold px-6 py-4 rounded-xl flex items-center gap-3 transition-all duration-300 shadow-[0_0_20px_rgba(220,38,38,0.5)] hover:scale-105 text-sm md:text-xl text-center"
                        >
                            <FaYoutube className="text-3xl" />
                            Visit Noor Productions YouTube Channel for Full Videos
                        </a>
                    </div>
...
<section className="bg-white py-16 border-t">
  <div className="container mx-auto">
    <Gallery 
      title="🎙️ Interviews with Important Personalities" 
...
<section className="bg-gray-50 py-16 border-t">
  <div className="container mx-auto">
    ...
    />
            ...
            <section className="container mx-auto px-4 pb-16 pt-10">
                <div className="max-w-4xl mx-auto bg-gradient-to-br from-[#0b314d] to-[#0f4c75] rounded-3xl p-8 md:p-12 text-white border-2 border-[#D4AF37] shadow-2xl text-center relative overflow-hidden">
                    ...
                    <div className="relative z-10">
                        <h2 className="text-2xl md:text-4xl font-bold mb-3 text-[#D4AF37]">Follow Us</h2>
                        <p className="text-sm md:text-xl mb-8 opacity-90 font-light">Join us on social media for all new updates and exclusive content</p>
            ...
            {activeVideo && (
                <div className="fixed inset-0 z-[1000] bg-black/95 flex items-center justify-center p-4 backdrop-blur-md" onClick={() => {
                    setActiveVideo(null);
...
export default function TalkshowsPageEN() {
    return (
