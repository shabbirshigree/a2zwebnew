...
const UrduFlipBook = dynamic(() => import('./UrduFlipBook'), {
  ...
  loading: () => (
    <div className="flex flex-col items-center justify-center p-20 gap-4">
      ...
      <div className="text-[#D4AF37] urdu-text text-center text-xl">انتظار فرمائیے...</div>
      <div className="text-[#D4AF37] urdu-text text-sm opacity-80">حاجی شبیر احمد شگری</div>
    </div>
  )
});
          ...
          >
            ...
            <div className="flex-1 text-right flex flex-col justify-center">
              <div className="flex items-center gap-3 mb-4">
                <div className="bg-[#D4AF37] text-black text-[10px] font-bold px-3 py-1 rounded-full">{book.badge}</div>
                <h3 className="text-2xl md:text-3xl font-extrabold text-[#D4AF37] urdu-text group-hover:translate-x-[-10px] transition-transform duration-500">{book.title}</h3>
              </div>
              <p className="text-gray-400 text-sm md:text-lg leading-loose urdu-text text-justify font-light opacity-80 group-hover:opacity-100 transition-opacity">
                {book.descUrdu}
              </p>
            </div>
          </div>
      ...
      {bookModalOpen && (
        <div className="fixed inset-0 z-[2000] bg-black/95 backdrop-blur-xl flex flex-col">
          <div className="flex justify-between items-center p-4 border-b border-[#D4AF37]/20 bg-black/50">
            ...
            <h2 className="text-[#D4AF37] urdu-text text-xl font-bold">{activeBookTitle}</h2>
