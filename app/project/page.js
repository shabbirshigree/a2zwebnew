    ...
    return (
      <section id={id} className="py-8 md:py-12 relative z-10 px-4 border-t border-white/5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 max-w-6xl mx-auto">
          {videos.slice(0, visibleCount).map((video) => <VideoCard key={video.id} video={video} />)}
        </div>
    ...
    <main className="min-h-screen bg-[#050505] text-white overflow-x-hidden font-sans">
      ...
      {showBooklet && (
        <div className="fixed inset-0 bg-black/95 z-[100] flex flex-col items-center justify-center p-4 backdrop-blur-sm" onClick={() => setShowBooklet(false)}>
           <div className="w-full max-w-5xl h-[80vh] relative flex flex-col items-center animate-in zoom-in-95 duration-300" onClick={e => e.stopPropagation()}>
              <button className="absolute -top-12 right-0 bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-full flex items-center gap-2 text-sm font-bold transition-all shadow-md border border-white z-50 urdu-text" onClick={() => setShowBooklet(false)}>
                <FaTimes /> بند کریں
              </button>
              <div className="w-full h-full bg-white rounded-2xl p-1 shadow-[0_0_50px_rgba(212,175,55,0.4)] overflow-hidden">
                  <iframe allowFullScreen="allowfullscreen" allow="clipboard-write" scrolling="no" src="https://heyzine.com/flip-book/e92a02b4a9.html" className="w-full h-full rounded-xl"></iframe>
              </div>
           </div>
        </div>
      )}
