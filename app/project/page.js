...
// --- یہ آپ کی اصلی فائلیں واپس آ گئی ہیں ---
...
export default function ProjectPage() {
  ...
  const VideoCard = ({ video }) => (
    <div onClick={() => setSelectedVideo(video)} className="bg-black rounded-lg overflow-hidden shadow-lg hover:shadow-2xl hover:scale-105 transition duration-300 cursor-pointer group border border-gray-800">
      ...
      <div className="bg-gray-900 p-3 text-center border-t border-gray-800"><p className="text-white font-serif text-sm leading-relaxed text-right">{video.title}</p></div>
    </div>
  );

  // --- ویڈیو گرڈ (بغیر سرچ کے) ---
  const VideoGrid = ({ title, videos, id }) => (
    <section id={id} className="py-12 relative z-10 px-4">
      <div className="text-center mb-10">
        <div className="inline-block bg-[#0f4c75] text-[#D4AF37] px-8 py-3 rounded-full border-2 border-[#D4AF37] font-serif font-bold text-lg shadow-lg">{title}</div>
      </div>
  ...
  return (
    <main className="min-h-screen bg-black text-white">
      {/* --- آپ کا اصلی ہیڈر اور سلائیڈر --- */}
      <Navbar />
      <HeroSlider />

      {/* --- بانی پراجیکٹ کا پیغام --- */}
      <section className="container mx-auto px-4 py-12 relative z-10">
        <div className="max-w-4xl mx-auto bg-white rounded-[2rem] shadow-2xl border-4 border-[#D4AF37] p-8 md:p-12 text-black">
          <h2 className="text-2xl font-bold text-[#0f4c75] text-center font-serif mb-6">بانی پراجیکٹ کا پیغام</h2>
          <div className="bg-yellow-50 border-r-4 border-[#D4AF37] p-6 mb-8 text-center font-serif text-lg font-bold text-gray-800" dir="rtl">
             "قرآن کو اونچے طاق میں رکھ دینا اس کا احترام نہیں، بلکہ اسے سمجھ کر، غوروفکر کر کے اس پر عمل کرنا ہی اس کا حقیقی احترام ہے۔"
          </div>
          <div className="text-right space-y-4 font-serif" dir="rtl">
             <p className="text-gray-700 leading-relaxed text-lg">اللہ کے فضل و کرم سے <strong>"نورالقرآن پراجیکٹ"</strong> کا آغاز ایک خواب کی تعبیر ہے۔ اس کا مقصد یہ ہے کہ قرآن کریم کو جدید <strong>سمعی و بصری</strong> انداز میں پیش کیا جائے تاکہ نوجوان نسل قرآن کو <strong>دل کی آنکھ سے دیکھنے اور سمجھنے لگیں۔</strong></p>
             <div className="bg-blue-50 border-r-4 border-[#0f4c75] p-4 my-4">
                 <p className="text-[#0f4c75] font-bold">یہ محض ایک تخلیقی آئیڈیا نہیں بلکہ خلوصِ نیت سے اٹھایا ہوا عملی قدم ہے۔</p>
             </div>
             <p className="text-[#0f4c75] font-bold text-lg text-left mt-4 border-b-2 border-[#D4AF37] inline-block pb-1">حاجی شبیر احمد شگری (بانی)</p>
          </div>
        </div>
      </section>
      ...
      {/* --- افتتاحی ویڈیوز --- */}
      <section className="container mx-auto px-4 py-12 relative z-10">
        <div className="bg-gradient-to-r from-[#D4AF37] to-[#ffd700] rounded-2xl shadow-2xl p-8 mb-12 text-black">
          <h3 className="text-2xl font-bold text-[#0f4c75] text-center font-serif mb-4">✨ نور القرآن پروجیکٹ کا باقاعدہ افتتاح ✨</h3>
          <p className="text-center text-gray-900 font-serif mb-6 text-lg">یکم رمضان المبارک کو <strong>علامہ شیخ محسن علی نجفیؒ</strong> کی قبر مطہر پر (جامعہ کوثر اسلام آباد) میں اس تاریخی پروجیکٹ کا افتتاح۔</p>
          <div className="aspect-video rounded-xl overflow-hidden shadow-xl border-4 border-white"><iframe src="https://www.youtube.com/embed/ah0OXlnDw2k" allowFullScreen className="w-full h-full"></iframe></div>
        </div>

        <div className="grid md:grid-cols-2 gap-8 mb-12">
          <div className="bg-white rounded-xl shadow-xl overflow-hidden hover:shadow-2xl transition group">
            ...
            <div className="p-4 bg-gray-100 group-hover:bg-white transition"><h4 className="font-serif font-bold text-gray-800 text-center">دنیا و آخرت کی کامیابی کے لئے قرآن سے رشتہ مضبوط کریں</h4></div>
          </div>
          <div className="bg-white rounded-xl shadow-xl overflow-hidden hover:shadow-2xl transition group">
            ...
            <div className="p-4 bg-gray-100 group-hover:bg-white transition"><h4 className="font-serif font-bold text-gray-800 text-center">قرآن پاک کی آڈیو ویڈیو جدید انداز میں</h4></div>
          </div>
        </div>
      </section>
      ...
      {selectedVideo && (
        <div className="fixed inset-0 bg-black/95 z-50 flex items-center justify-center p-4 backdrop-blur-sm" onClick={() => setSelectedVideo(null)}>
          <div className="relative w-full max-w-4xl" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="text-[#D4AF37] text-center mt-4 text-xl font-serif font-bold">{selectedVideo.title}</p>
          </div>
        </div>
      )}
