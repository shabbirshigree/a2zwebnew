...
export default function Project() {
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-gray-900 via-black to-gray-900">
      ...
      {/* Search Bar */}
      <div className="bg-gradient-to-r from-[#0f4c75] to-[#0a3552] py-6 sticky top-20 z-20">
        <div className="container mx-auto px-4 flex justify-end">
          <div className="relative w-full md:w-64">
            <FaSearch className="absolute right-3 top-3 text-[#D4AF37]" />
            <input
              type="text"
              placeholder="تلاش کریں..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full bg-white text-right pr-10 pl-4 py-2 rounded-full border-2 border-[#D4AF37] focus:outline-none focus:shadow-lg font-serif"
            />
          </div>
        </div>
      </div>
